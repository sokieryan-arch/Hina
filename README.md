...
- Configure a Paddle webhook endpoint at `https://hina-gules.vercel.app/api/paddle/webhook`, subscribe at least to `transaction.paid`, `subscription.activated`, `subscription.updated`, `subscription.canceled`, and `subscription.paused`, then save the webhook secret as `PADDLE_WEBHOOK_SECRET` in Vercel.
- Paddle webhooks unlock Pro by writing to the same server-side billing store used for quota enforcement. In production, Firebase Admin credentials are required so webhook updates persist across serverless invocations.
