...
import { findWritingTask2Prompt } from "../practice/writingPrompts.js";
import type {
  LanguageCode,
  SpeakingStudyCard,
...
export function readWritingEvaluationInput(input: unknown): WritingEvaluationInput {
  ...
  if (!findWritingTask2Prompt(questionId)) invalid("Unknown writing question.");
...
export function buildWritingEvaluationPrompt(input: WritingEvaluationInput) {
  const prompt = findWritingTask2Prompt(input.questionId);
  ...
  return `You are Hina acting as a conservative IELTS Writing Task 2 practice evaluator, not an official examiner.
...
Evaluation contract:
- Score Task Response, Coherence and Cohesion, Lexical Resource, and Grammatical Range and Accuracy from 0 to 9 in half-band steps.
...
- A response under 250 words is underlength. The server will apply a conservative evidence ceiling, so do not compensate with generous scores.
...
export function writingScoreCeiling(wordCount: number) {
...
function evidenceConfidence(wordCount: number, scoreCeiling: number | null) {
  if (scoreCeiling !== null) return "low" as const;
  return wordCount >= 320 ? "high" as const : "medium" as const;
}

function underlengthSummary(language: LanguageCode, wordCount: number, scoreCeiling: number) {
  const ceiling = scoreCeiling.toFixed(1);
  const summaries: Record<LanguageCode, string> = {
    en: `This draft contains ${wordCount} words. It is under the 250-word Task 2 minimum, so there is not enough developed evidence for a higher estimate; the practice score is capped at ${ceiling}.`,
    "zh-CN": `这篇作文共有 ${wordCount} 词，低于 Task 2 的 250 词要求，论证证据不足，因此本次练习估分最高限制为 ${ceiling}。`,
    ja: `この作文は ${wordCount} 語で、Task 2 の最低 250 語を下回っています。十分に展開された根拠がないため、今回の推定上限は ${ceiling} です。`,
    ko: `이 글은 ${wordCount}단어로 Task 2 최소 기준인 250단어보다 짧습니다. 충분히 전개된 근거가 없어 이번 연습 점수는 최대 ${ceiling}로 제한됩니다.`,
    es: `Este texto tiene ${wordCount} palabras y no alcanza el mínimo de 250 para Task 2. Falta evidencia desarrollada, por lo que la estimación queda limitada a ${ceiling}.`,
    pt: `Este texto tem ${wordCount} palavras e não atinge o mínimo de 250 da Task 2. Falta desenvolvimento suficiente, por isso a estimativa fica limitada a ${ceiling}.`,
    fr: `Ce texte compte ${wordCount} mots et reste sous le minimum de 250 mots de la Task 2. Les idées ne sont pas assez développées ; l'estimation est donc plafonnée à ${ceiling}.`,
    de: `Dieser Text umfasst ${wordCount} Wörter und liegt unter dem Minimum von 250 Wörtern für Task 2. Die Ideen sind nicht ausreichend entwickelt; die Schätzung ist daher auf ${ceiling} begrenzt.`,
  };
...
export function normalizeWritingEvaluation(
  input: unknown,
  context: { essay: string; nativeLanguage?: LanguageCode },
): WritingEvaluation {
  ...
  const scoreCeiling = writingScoreCeiling(wordCount);
  ...
  const scores: WritingScores = {
    taskResponse: cap(rawScores.taskResponse),
  ...
  return {
    summary: scoreCeiling === null
      ...
      : underlengthSummary(context.nativeLanguage || "en", wordCount, scoreCeiling),
    estimatedBand,
    scores,
    ...
    evidence: {
      wordCount,
      scoreCeiling,
      confidence: evidenceConfidence(wordCount, scoreCeiling),
    },
  };
}
