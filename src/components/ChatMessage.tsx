...
import { CheckCircle2, Lightbulb, MessageCircle, UserCircle, Volume2, Sun } from "lucide-react";
...
export function ChatMessage({ message, isSpeaking, onPlayAudio, userPhotoUrl }: ChatMessageProps) {
  ...
  const isTip = message.type === "tip" || message.type === "correction" || message.type === "insight";
  const isCorrectionTip = (message.type === "tip" && message.tipKind === "correction") || message.type === "correction";
  const isExpressionTip = (message.type === "tip" && message.tipKind !== "correction") || message.type === "insight";
  ...
  return (
    ...
    >
      <div className="flex-shrink-0">
        {isUser ? (
          <div className="w-10 h-10 rounded-full bg-[#5A5A40] dark:bg-[#48285c] border border-transparent dark:border-[#5c3773] flex items-center justify-center text-white shadow-sm transition-colors duration-300 overflow-hidden">
            {userPhotoUrl ? (
          ...
          <motion.div
            ...
            className={cn(
              ...
              message.type !== "response" ? "bg-[#FFD166] dark:bg-[#660874] text-black dark:text-white" : "bg-[#FFD166] text-white",
            )}
          >
            {isTip
              ? (isCorrectionTip ? <CheckCircle2 size={22} /> : <Lightbulb size={22} />)
              : (message.type === "proactive" ? <MessageCircle size={22} /> : <Sun size={24} strokeWidth={2.5} />)}
          </motion.div>
        )}
      </div>
      ...
      <div className={cn("space-y-2", isUser ? "text-left" : "")}>
        <div className={cn(
          ...
          {message.type === "proactive" && (
            <p className="text-xs font-bold uppercase tracking-widest mb-1 opacity-70">Hina popped in</p>
          )}
